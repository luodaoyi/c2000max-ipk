...
- 默认源码仓库 `shidahuilang/openwrt-package`
...
之所以这样配置，是因为 `shidahuilang/openwrt-package` 当前并不存在 `luci-app-openclash` 目录，但包含 `luci-app-zerotier`。
...
- `config/target.env`
  - 固定目标版本、架构、SDK 镜像和默认源码仓库参数
- `config/packages.txt`
  - 默认待编译包列表，支持“包目录 + 源码仓库 + 分支/标签”三段式配置
